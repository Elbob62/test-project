...
// مصفوفة البيانات بـ 12 صندوق ورسائلك الجديدة
const boxesData = [
    ...
    { type: 'romantic', messages: ["العالم كله في كفة، وانتي لوحدك في كفة تانية خالص", "هفضل طول عمري صادق معاكي ف ي كل كلمه وعمر ماخون وعودنا ولا اتفقنا ❤️", "عوض ربنا ليا كان في ضحكتك"] },
    { type: 'mixed', messages: ["لو بطلنا نتخانق هنموت من الملل، بس المهم نفضل نحب بعض بعد كل خناقة! 🥊❤️","بحبك يا مجنونة بس برضه بتعصبيني! ❤️😂", "انتي أحلى حاجة وأرخم حاجة في حياتي", "انتي السند والظهر، وانتي اللي بتخليني أحس إن الدنيا لسه فيها حاجات حلوة. ✨"] },
    { type: 'me', messages: ["انا اسف عشان زعلتك وخوفتك حقك عليا😔🫂", "انا بابا يعني الامان والسند مقدرش ع زعلك ولا وجعك ولا اني اشوفك تايهه او اشوف فيكي اي حاجه وحشه انتي روح بابا حقك عليا 🫂🫂", "هعملك كل الي تطلبيه وكل الي نفسك فيه وكل الي عايزاه وكل الي ترتاحي فيه ميهمنيش غير راحتك واشوفك مطمنه وكويسه وحاسه دايما بفرحه والامان❤️❤️🫂🫂🫂🫂"] },
    { type: 'tryAgain', messages: ["جربي حظك تاني يا قمر! 🔄", "الصندوق ده فاضي، شوفي غيره 😜", "يلا من هنا ي شطره هنرش ميه😂"] },
    { type: 'tryAgain', messages: ["تؤ تؤ.. مش ده! بس ضحكتك وانتي بتجربي أحلى من أي مفاجأة 😍","بلاش طمع.. افتحي الصناديق اللي جنبه! 🎁","يا بنتي قولتلك الصندوق الصح مش هنا.. ركزي يا ميرنا! 🧐","لصندوق ده فاضي زي جيبي قبل القبض.. شوفي غيره! 💸😂"] },
    { type: 'funny', messages: ["انتي 'الراوتر' بتاع حياتي.. من غيرك أنا 'Offline' ومش فاهم حاجة! 📶😂","عارفة إيه المشترك بينك وبين الحكومة؟.. إنكم دايماً على حق وأنا اللي غلطان! 🚔🫡","لو خيروني بينك وبين صينية بسبوسة.. هختارك انتي طبعاً (بس البسبوسة هتفضل في قلبي برضه) 😂","انتي 'الشاحن الأصلي' اللي بيرجعلي طاقتي أول ما بشوفك 🔋"] },
    { type: 'mixed', messages: ["انتي 'النكد' اللي بعشقه، والروتين اللي مستعد أعيشه كل يوم من غير زهق! ❤️😂","انتي 'البانادول' بتاع صداعي، بس انتي برضه اللي جايبالي الصداع! ❤️🤡","يا نصي الحلو.. والنص التاني اللي بيجنني! 🌓","حبك زي 'الواي فاي'.. ملوش شكل بس من غيره حياتي واقفة خالص. 📶"] },
    { type: 'me', messages: ["انا مستحملش اشوف زعلك او خنقتك او خوفك او قلقك وببقا عايز اولع ف الي عمل كده ف عمري ماقدر اني اشوفك كده وكمان بسببي😔😔","بابا ميقدرش يشوف حاجه ف بنوته حقك عليا 🫂","انتي غاليه عليا وانتي عاعرفه انتي اي بالنسبه ل حسن وعارفه ان حسن مستعد يعمل اي حاجه ف دنيا لاجلك بس تكون برضاكي وعشانك عشان تبقي ف امان ومطمنه متخفيش ي ماما انا الامان والله","تعااااااااااااااالي ف حضني 🫂🫂🫂🫂🫂🫂🫂🫂🫂🫂🫂🫂"] },
    { type: 'me', messages: ["اطمني ي بنوتي عمري ماهسيبك ولا ابعد عنك ولا اقدر احطك ف مقارنه مع حد مهما كان مين انتي وبس🫂","بس احي اقولك برده ان مش هعمل غير الي يريحك ويكون ف صالحك وراحتك وامانك وفرحتك ده الاساس انك دايما ف امان وفرحه ودلع وكل حاجه خير🫂🫂🫂🫂🫂"] }
];

// مصفوفة لتتبع عدد الضغطات لكل صندوق بشكل مرن
let clickCounts = new Array(boxesData.length).fill(0);

function initGame() {
    const grid = document.getElementById('boxesGrid');
    if (!grid) return;

    // خلط الصناديق عشوائياً عند كل ريفريش
    const shuffled = [...boxesData].sort(() => Math.random() - 0.5);

    shuffled.forEach((info, index) => {
        ...
        box.onclick = function() {
            // تشغيل الصوت
            const sound = document.getElementById('openBoxSound');
            if(sound) sound.play();

            const display = document.getElementById('gameResponse');
            
            // اختيار الرسالة بناءً على عدد الضغطات
            let msgArray = info.messages;
            let currentClick = clickCounts[index];
            
            // معادلة ذكية تختار الرسالة اللي عليها الدور وتلف تاني لو خلصوا
            let messageToShow = msgArray[currentClick % msgArray.length];
            
            display.innerText = messageToShow;
            
            // زيادة العداد الخاص بهذا الصندوق
            clickCounts[index]++;

            // تأثير بصري بسيط عند الضغط
            box.style.transform = "scale(0.9)";
            setTimeout(() => { box.style.transform = "translateY(-10px) scale(1.05)"; }, 100);
        };
